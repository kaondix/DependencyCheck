...
/**
 * Used to analyze a mix package, and collect information that can be used to
 * determine the associated CPE.
 *
 * @author - our Customer TODO: get author
 */
@Experimental
@ThreadSafe
public class MixPackageAnalyzer extends AbstractFileTypeAnalyzer {

    /**
     * A descriptor for the type of dependencies processed or added by this
     * analyzer.
     */
    public static final String DEPENDENCY_ECOSYSTEM = "Mix.Pkg";
     ...
     */
    private static final String EXTENSIONS = "exs";

    /**
     * Filter that detects files named "mix.exs".
     */
    private static final FileFilter MIX_EXS_FILTER = new NameFileFilter("mix.exs");

    /**
     * The file filter for mix files.
     */
    private static final FileFilter EXS_SUFFIX = new SuffixFileFilter(".exs");
    ...
    /**
     * Returns the name of the MIX Package Analyzer.
     *
     * @return the name of the analyzer
     */
    @Override
    public String getName() {
        return "Mix Package Analyzer";
    }
    ...
    protected String getAnalyzerEnabledSettingKey() {
        return Settings.KEYS.ANALYZER_MIX_PACKAGE_ENABLED;
    }
    ...
    private static Pattern compileAssignPattern(String name) {
        return Pattern.compile(
                String.format("\\b%s\\b *: *(['\"]+)(.*?)\\3", name),
                REGEX_OPTIONS);
    }

    /**
     * Analyzes mix packages and adds evidence to the dependency.
     *
    ...
    protected void analyzeDependency(Dependency dependency, Engine engine)
        ...
        if (MIX_EXS_FILTER.accept(file)) {
            ...
            final File[] fileList = parent.listFiles(EXS_SUFFIX);
            if (fileList != null) {
    ...
    private boolean analyzeFileContents(Dependency dependency, File file)
        ...
        if (!contents.isEmpty()) {
            ...
            found = gatherEvidence(dependency, EvidenceType.VERSION, Pattern.compile("\\b?@version?( *)( +)(['\"]+)(\\d+\\.\\d+.*?)(['\"]+).*",
                    REGEX_OPTIONS), contents,
            ...
            found |= addSummaryInfo(dependency,
                    Pattern.compile("\\b?description?( *):( +)(['\"]+)(.*?)(['\"]+).*",
                    REGEX_OPTIONS), 4, contents,
                    source, "summary");

            found |= gatherEvidence(dependency, EvidenceType.PRODUCT, Pattern.compile("\\b?name?( *):( +)(['\"]+)(.*?)(['\"]+).*",
                    REGEX_OPTIONS), contents,
                    source, "SourceTitle", Confidence.LOW);

            found |= gatherEvidence(dependency, EvidenceType.VENDOR, Pattern.compile("\\b?maintainers?( *):( +[\\[+])(['\"]+)(.*?)(['\"]+).*",
                    REGEX_OPTIONS), contents,
                    source, "SourceAuthor", Confidence.MEDIUM);
            found |= gatherHomePageEvidence(dependency, EvidenceType.VENDOR, Pattern.compile("\\b?source_url?( *):( +)(['\"]+)(.*?)(['\"]+).*",
                    REGEX_OPTIONS),
                    source, "URL", contents);
            found |= gatherHomePageEvidence(dependency, EvidenceType.VENDOR, Pattern.compile("\\b?source_url?( *):( +)(['\"]+)(.*?)(['\"]+).*",
                    REGEX_OPTIONS),
    ...
    /**
     * Gather evidence from a mix source file using the given string
     * assignment regex pattern.
     *
     * @param dependency the dependency that is being analyzed
     * @param type       the type of evidence
     * @param pattern    to scan contents with
     * @param contents   of mix source file
