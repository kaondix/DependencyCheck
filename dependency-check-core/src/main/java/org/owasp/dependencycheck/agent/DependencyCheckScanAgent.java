...
public class DependencyCheckScanAgent {
    ...
    private void populateSettings() {
        ...
        setSettingsString(Settings.KEYS.PROXY_SERVER, proxyServer);
        setSettingsString(Settings.KEYS.PROXY_PORT, proxyPort);
        setSettingsString(Settings.KEYS.PROXY_USERNAME, proxyUsername);
        setSettingsString(Settings.KEYS.PROXY_PASSWORD, proxyPassword);
        setSettingsString(Settings.KEYS.CONNECTION_TIMEOUT, connectionTimeout);
        setSettingsString(Settings.KEYS.SUPPRESSION_FILE, suppressionFile);
        Settings.setBoolean(Settings.KEYS.ANALYZER_CENTRAL_ENABLED, centralAnalyzerEnabled);
        setSettingsString(Settings.KEYS.ANALYZER_CENTRAL_URL, centralUrl);
        Settings.setBoolean(Settings.KEYS.ANALYZER_NEXUS_ENABLED, nexusAnalyzerEnabled);
        setSettingsString(Settings.KEYS.ANALYZER_NEXUS_URL, nexusUrl);
        Settings.setBoolean(Settings.KEYS.ANALYZER_NEXUS_USES_PROXY, nexusUsesProxy);
        setSettingsString(Settings.KEYS.DB_DRIVER_NAME, databaseDriverName);
        setSettingsString(Settings.KEYS.DB_DRIVER_PATH, databaseDriverPath);
        setSettingsString(Settings.KEYS.DB_CONNECTION_STRING, connectionString);
        setSettingsString(Settings.KEYS.DB_USER, databaseUser);
        setSettingsString(Settings.KEYS.DB_PASSWORD, databasePassword);
        setSettingsString(Settings.KEYS.ADDITIONAL_ZIP_EXTENSIONS, zipExtensions);
        setSettingsString(Settings.KEYS.CVE_MODIFIED_12_URL, cveUrl12Modified);
        setSettingsString(Settings.KEYS.CVE_MODIFIED_20_URL, cveUrl20Modified);
        setSettingsString(Settings.KEYS.CVE_SCHEMA_1_2, cveUrl12Base);
        setSettingsString(Settings.KEYS.CVE_SCHEMA_2_0, cveUrl20Base);
        setSettingsString(Settings.KEYS.ANALYZER_ASSEMBLY_MONO_PATH, pathToMono);
    }

    private void setSettingsString(String settingsKey, String settingsValue) {
        if (null != settingsValue && !settingsValue.isEmpty()) {
            Settings.setString(settingsKey, settingsValue);
        }
    }
